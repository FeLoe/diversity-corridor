...
news = read_csv(here::here("data/raw/news.csv"))
...
#Question: What would be a useful diversity measurement for this? 
...
#What are other options of computing that variable?
#Maybe: Including the 4 into the mix (on the middle of the scale for the news topic) -> quite substantially changes the distribution
...
write.csv(qualtrics_not_finished, file = "datasets/qualtrics_dropout.csv", row.names = FALSE)
...
#Excluding respondents who have a SD of 0 on one of the dependent variables
...
write.csv(qualtrics_stories_w2, file = "datasets/qualtrics_stories_w2.csv", row.names = FALSE)
...
write.csv(news_dropout, file = "datasets/news_dropout.csv", row.names = FALSE)
write.csv(qualtrics_finished, file = "datasets/qualtrics_finished.csv", row.names = FALSE)
...
####TODO: Find out if users have in general preferences for one specific position (always select the first?); that also depends on most 
#used device (rather a mobile user etc). Make groups of "always the first" selectors etc. + Still needed: PERCENTAGE 
...
###Make different datasets for Martin: User information, exposure information, article information 
names(exposures_users)
...
write.csv(users, file = (here::here("data/tmp/users_new.csv")), row.names = FALSE)
write.csv(article_info, file = (here::here("data/tmp/article_features.csv")), row.names = FALSE)
write.csv(exposures, file = (here::here("data/tmp/exposures.csv")), row.names = FALSE)

#TODO Incorporate 3rd wave material with the final dataset



# #___________________________________________________________Merging with third dataset_______________________________________________________________________
# #Merging with third dataset (after adding control)
# qualtrics_third_wave = merge(x = qualtrics_finished, y = qualtrics_third, by = "id")
# names(qualtrics_third_wave)
# 
# #Cronbach's alpha for the variables (1) system control, (2) strategy control (3) perceived diversity and (4) website satisfacton (adjectives)
# 
# #system control: alpha 0.83
# #strategy control: alpha 0.83
# #diversity: alpha 0.71
# #satisfaction: alpha 0.95
# 
# omega(qualtrics_third_wave[c("Q14_1", "Q14_2", "Q14_3", "Q14_4")], check.keys=TRUE)
# 
# omega(qualtrics_third_wave[c("Q11_1", "Q11_2", "Q11_3", "Q11_4", "Q11_5")], check.keys=TRUE)
# 
# omega(qualtrics_third_wave[c("Q3_1", "Q3_2", "Q3_3","Q3_4")], check.keys=TRUE)
# 
# omega(qualtrics_third_wave[c("Q1_1.y","Q1_2.y","Q1_3.y","Q1_4.y", "Q1_5.y", "Q1_6.y", "Q1_7.y")], check.keys=TRUE)
# 
# #Recoding the items that were reverse coded 
# 
# cols = c("Q14_1", "Q11_1", "Q11_2", "Q11_3", "Q3_2", "Q3_4")
# qualtrics_third_wave[ ,cols] = 8 - qualtrics_third_wave[ ,cols]
# 
# 
# #Making the scales for the DVs (not making one for diversity yet)
# 
# qualtrics_third_wave = qualtrics_third_wave %>% mutate(system_control_third = rowMeans(.[, c("Q14_1", "Q14_2", "Q14_3", "Q14_4")]))
# 
# qualtrics_third_wave = qualtrics_third_wave %>% mutate(strategy_control_third = rowMeans(.[, c("Q11_1", "Q11_2", "Q11_3", "Q11_4", "Q11_5")]))
# 
# qualtrics_third_wave = qualtrics_third_wave %>% mutate(satisfaction_third = rowMeans(.[, c("Q1_1","Q1_2","Q1_3","Q1_4", "Q1_5", "Q1_6", "Q1_7")]))
# 
# #Difference score 
# 
# qualtrics_third_wave$system_control_diff = qualtrics_third_wave$system_control_third - qualtrics_third_wave$system_control
# qualtrics_third_wave$strategy_control_diff = qualtrics_third_wave$strategy_control_third - qualtrics_third_wave$strategy_control
# qualtrics_third_wave$satisfaction_diff = qualtrics_third_wave$satisfaction_third - qualtrics_third_wave$satisfaction
# 
# #Also regarding third wave: How often were diversity/num_recommender control sliders used? 
# div_stats = diversity %>% group_by(user_id) %>% summarise(number = n(), distinct = n_distinct(real), sd = sd(real), mean = mean(real))
# describe(div_stats)
# table(div_stats$distinct)
# table(div_stats$mean)
# 
# #Results: In total, 62 people used the diversity slider, they used it between 1 and 8 times, M = 2.15, SD = 1.56
# #They used between 1 and 4 different settings, M = 1.58, SD = 0.84, two people did it four times, 8 people 3 times, 61% only used it once
# #They put the slider on average on M = 2.63 (SD = 1.25), ranging between 1 (17 people) and 5 (3 people). This average is higher to the original settings (1)
# #Conclusion: 27% left it at the original position (low diversity), 4 people put it on average on higher than 4. 
# #Most people tried to increase the diversity, however when looking at the average they did also not try to maximize it (mostly staying at 2-3 on a 5 point scale)
# 
# num_rec_stats = num_rec %>% group_by(user_id) %>% summarise(number = n(), distinct = n_distinct(real), sd = sd(real), mean = mean(real))
# describe(num_rec_stats)
# table(num_rec_stats$distinct)
# table(num_rec_stats$mean)
# 
# #Results: In total, 84 people used the num_recommended slider, they used it between 1 and 8 times, M = 2.02, SD = 1.40
# #They used between 1 and 4 different settings, M = 1.54, SD = 0.74, two people did it four times, 6 people 3 times, 58% only used it once
# #They put the slider on average on M = 6.51 (SD = 1.72), ranging between 0 (only 1 person) and 9 (14 people). This average is close to the original settings (6), which was also most often used by people (27)
# #Conclusion: They often stay with the standard options (most left the slider at 6) or go to the highest extreme (9) which means everything should be personalised
# #The lower options (i.e. less personalised) were used by 15 people (17% of those who used the slider at least once). So most people that used the tool opted for same amount/more personalisation
# 
# num_selected = selected_news_info %>% filter(user_id %in% qualtrics_sql$id.y) %>% group_by(user_id) %>% summarise( n = n())
# psych::describe(num_selected)
# 
# save(qualtrics_finished, file = "qualtrics_finished.Rda")
# write.csv(qualtrics_third_wave, file = "qualtrics_third_wave.csv")
# 
# #Manual recoding of two columns (payment for news in general and for this website) in Excel
# 
# headers = read_delim("qualtrics_third_wave_rec.csv", delim = ";", col_names = FALSE, n_max = 1)
# qualtrics_third_wave = read_delim("qualtrics_third_wave_rec.csv", delim = ";", skip = 1, col_names = FALSE)
# colnames(qualtrics_third_wave)= headers
# 
# save(qualtrics_third_wave, file = "qualtrics_third_wave.Rda")
# describe(qualtrics_third_wave$payment_recoded)
# describe(qualtrics_third_wave$News_payment)
# table(qualtrics_third_wave$payment_recoded)
# table(qualtrics_third_wave$News_payment)
# 
# group.labs <-c("Recommender Topic (n = 69)", "Customization (n = 43)")
# names(group.labs) <-  c("3", "4")
# 
# id_group = qualtrics_stories_w1 %>% select(group, user_id, id) %>% filter(id %in% qualtrics_finished$id)  %>% select(-id) %>% distinct(group, user_id, .keep_all = TRUE) 
# category_p_p = dplyr::right_join(category, id_group, by = "user_id")
# category_p_p = category_p_p %>% filter(group == "Topic Rec" | group == "Customization") %>% group_by(user_id) %>% summarise(n = n(), group = first(group), topic1 = first(topic1))
# category_p_p$n = ifelse(category_p_p$n == 1 & is.na(category_p_p$topic1), 0, category_p_p$n)
# table(category_p_p$group, category_p_p$n)
# category_p_p %>% filter(group == "Customization") %>% filter(n == 0)
# 
# ggplot(category_p_p) +
#   geom_bar(aes(n)) + 
#   ggtitle("How often did participants use the customization option (adjusting topic preferences)?") + ylab("Number of Participants")+ 
#   xlab("Number of category selections") + facet_wrap(~ group, labeller = labeller(group = group.labs))
